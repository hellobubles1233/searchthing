...
export class Footer {
  ...
  constructor() {
    // Create footer element with Tailwind classes - Notion-style
    this.footer = createElement('footer', { 
      // Use text-light color
      className: 'mt-10 text-center text-xs text-var(--text-light) p-4 fade-in' // Use CSS var
    });
    ...
    links.forEach(link => {
      const linkEl = createElement('a', { 
        href: link.href,
        target: '_blank',
        // Use text-light and standard hover
        className: 'flex items-center text-var(--text-light) hover:text-var(--text-color) transition-colors hover:bg-var(--bg-light) rounded px-2 py-1' // Use CSS vars
      });
    ...
    const copyright = createElement('div', {
      className: 'text-xs text-var(--text-light) mt-2' // Use CSS var
    ...
    const unduckLink = createElement('a', {
      ...
      target: '_blank',
      className: 'text-var(--text-light) hover:text-var(--text-color) transition-colors underline' // Use CSS vars
    }, ['Unduck']);

    const rebangLink = createElement('a', {
      href: 'https://github.com/Void-n-Null/rebang',
      target: '_blank',
      className: 'text-var(--text-light) hover:text-var(--text-color) transition-colors underline ml-1' // Use CSS vars
    }, ['ReBang']);
